...
for indxDist = 1: length(vDist)
    ...
    useClearType = true;
    scene{1} = ctCreateFontScene(vDisp, dist, stim, useClearType);
    scene{1} = sceneSet(scene,'name',sprintf('ClearType-%.0f',dpi));

    useClearType = false;   
    scene{2} = ctCreateFontScene(vDisp, dist, stim, useClearType);
    scene{2} = sceneSet(scene,'name',sprintf('ClearType-%.0f',dpi));
